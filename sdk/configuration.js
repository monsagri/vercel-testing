...
const vercel = require('./edgeConfigStore');
...
module.exports = (function () {
  ...
  const validate = function (edgeConfig, sdkKey, options) {
    if (!sdkKey) {
      throw new Error(messages.missingKey());
    }

    if (!edgeConfig || !!edgeConfig.get === false) {
    ...
    if (!config.featureStore) {
      config.featureStore = vercel.VercelFeatureStore(edgeConfig, sdkKey, {}, config.logger);
    }
